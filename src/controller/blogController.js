...
const createBlog = async function (req, res){
    try{
        ...
        if (!blogDetails.title) {return res.status(400).send({status:false, msg: "Title is required"})}   //Title is mandory
        if (!blogDetails.body) {return res.status(400).send({status:false, msg: "Body is required"})}   //body is mandory
        if (!blogDetails.category) {return res.status(400).send({status:false, msg: "Category is required"})}  //category is mandory
        if (!blogDetails.authorId) {return res.status(400).send({status:false, msg: "AuthorId is required"})}  //authorId is mandory
